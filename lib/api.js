...
TidalAPI.prototype.createPlaylist = function(data) {
  var self = this;
  return this.getMyID().then(id => {

    return self._basePost('/users/' + id + '/playlists', {
      countryCode: _countryCode
    }, {
      title: data.title,
...
TidalAPI.prototype.getUser = function(user) {
    var self = this;
    return this.getMyID().then(id => {
        return self._baseRequest('/users/' + (id || user), {
    ...
    TidalAPI.prototype.getPlaylists = function(user) {
        var self = this;
        return this.getMyID().then(id => {
            return self._baseRequest('/users/' + (id || user) + "/playlists", {
              limit: user.limit || 999,
              offset: user.offset || 0,
              countryCode: _countryCode
            });
