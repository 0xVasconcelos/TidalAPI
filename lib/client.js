...
TidalAPI.prototype.getPlaylist = function(query) {
  var self = this;
  return self._baseRequest('/playlists/' + (query.id || query), {
    limit: query.limit || 999,
    filter: query.filter || 'ALL',
    offset: query.offset || 0,
    countryCode: _countryCode
...
TidalAPI.prototype.getPlaylistTracks = function(query) {
  var self = this;
  return self._baseRequest('/playlists/' + (query.uuid || query) + '/tracks', {
    limit: query.limit || 999,
    filter: query.filter || 'ALL',
    offset: query.offset || 0,
    countryCode: _countryCode
...
TidalAPI.prototype.addPlaylistTracks = function(data) {
  var self = this;
  return self._basePost('/playlists/' + data.uuid + '/items', {
    countryCode: _countryCode
  }, {
...
TidalAPI.prototype.deletePlaylistTracks = function(data) {
  var self = this;
  return self._baseDelete('/playlists/' + data.uuid + '/items/' + (data.trackPositions.join(',')), {
    countryCode: _countryCode,
    orderDirection: data.orderDirection || 'ASC',
    order: data.order || 'INDEX'
  }, { /*trackIds: tracks, toIndex: 1*/ }, '');
}
